...
def getBinEdges(parameterValues, **kwargs):
  ...
  **Fixed bin Count:**
  ...
  bin_upper_limit = kwargs.get('binWidthupper')
  bin_lower_limit = kwargs.get('binWidthlower')
  # print(bin_upper_limit, bin_lower_limit)
  ...
  if binCount is not None:
    ...
    minimum = bin_lower_limit
    maximum = bin_upper_limit
    # print(minimum, maximum)
    ...
    num_elements = int(numpy.round((highBound - lowBound) / binWidth))
    binEdges = numpy.linspace(lowBound, highBound, num_elements + 1, endpoint=True)
...
def binImage(parameterMatrix, parameterMatrixCoordinates=None, **kwargs):
  ...
  # import numpy as np
  # np.save('2.npz', discretizedParameterMatrix)
  # print(discretizedParameterMatrix)
