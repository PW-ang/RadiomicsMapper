...
from radiomics_customize import getProgressReporter, imageoperations
...
class RadiomicsFeaturesBase(object):
  ...
  def __init__(self, inputImage, inputMask, filename = None, **kwargs):
  ...
  def _applyBinning(self, matrix):
    ...
    if self.filename is not None:
      import numpy as np
      np.save(self.filename + '.npy', matrix)
    ...
    # print(self.coefficients['grayLevels'], self.coefficients['Ng'])
    return matrix
